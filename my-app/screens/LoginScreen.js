...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    if (!email || !password) return Alert.alert('Erro', 'Informe email e senha.');
    try { await login(email.trim(), password); navigation.replace('Tasks'); }
    catch (err) { Alert.alert('Erro no login', err.message); }
  };

  const handleRegister = async () => {
    if (!email || !password) return Alert.alert('Erro', 'Informe email e senha.');
    try { await register(email.trim(), password); navigation.replace('Tasks'); }
    catch (err) { Alert.alert('Erro no registro', err.message); }
  };
